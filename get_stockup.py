...
def get_stockup():
    try:
        ...
        links = soup.findAll("div" , {"class" : "Ta(s) W(20%)"})
        for i,link in enumerate(links): # links:
            if i in range(42,76):
                a_name.append(link.text)
                a_link.append(link.a.get("href"))
        ...
        for dnum,s_link in enumerate(a_link): # a_link:
            url = f"https://tw.stock.yahoo.com/{s_link}"
            # response = requests.get(url ,headers=headers_i ,verify=False)
            response = requests.get(url,headers=headers_i)
            response.encoding='utf-8' 
            if response.status_code == 200:
                ...
                for i,y in enumerate(stockid):
                    if "<" in y.text :
                        y_text = y.text
                        y_1 = y_text.text.replace(".TWO","")
                        if y_1 != "-" :
                            stockid_list.append(y_1)
                    else :
                        if y.text.replace(",","") != "-":
                            stockid_list.append(y.text.replace(".TWO",""))
        ...
        # print(df_all.head(5))
        # quit()
        df_real = df_all.query("stock_id.str.len() == 4 & stock_id.str.isnumeric()") 
...
# h_d1,h_d2,h_d3,h_vol_d1 = get_stockup()
