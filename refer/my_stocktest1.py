...
def cal_ratio(row):
    ...
    for one_d in date_range :
        ...
        # print(df)
            ...
            # df_long = df_copy.iloc[-60:]
            df_long = df_copy.iloc[-11,:]
            # print(df_long)
            df_long1 = df_long['over']
            # 最新over價
            df_newest = df_copy.iloc[-1,2]
            df_mean1 = round(df_copy[-11:].over.mean(),2)
            # 計算漲跌除平均乘100後取小數點2位
            ...
            # quit()
            # 個股差異50日
            # stockid = df_long['stockid'].iloc[0]
            # print(stockid)
            # stock_pct_change = df_long['over'].pct_change(diff_day)*100
            # print(stock_pct_change.iloc[-1])
            # quit()

            # if np.isnan(stock_pct_change.iloc[-1]) :
            #     pc = 0.0
            # else:
            #     # 取小數點後兩位
            #     pc = round(stock_pct_change.iloc[-1],2)
            # print(stock_pct_change)
            # print(stock_pct_change.shape)
            # print(pc)
            # quit()
...
df_rs['rs'] = (df_rs['rank']/len(df_rs))*100
